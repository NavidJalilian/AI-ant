...
function place_food() {
  ...
  for (var i = center_i - max_distance; i <= center_i + max_distance; i++) {
    for (
      var ii = center_ii - max_distance;
      ii < center_ii + max_distance;
      ii++
    ) {
  ...
  for (var i = 0; i < grid_length; i = i + 1) {
    for (var ii = 0; ii < grid_length; ii = ii + 1) {
      ...
      if (grid[i][ii].has_ant() && grid[i][ii].ant.has_food) {
        ...
        bounded_ii = get_bounded_index(ii);
        var signal_strength =
          1 - Math.pow(0.5, 1 / calc_distance(i, ii, bounded_i, bounded_ii));
...
function get_coords_from_orientation(i, ii) {
  ...
  coords.push(
    get_bounded_index(Math.round(ii + Math.sin(orientation_radians)))
  );
  return coords;
}
...
function calc_distance(i, ii, j, jj) {
  return Math.pow(
    Math.pow(Math.abs(i - j), 2) + Math.pow(Math.abs(ii - jj), 2),
    0.5
  );
}
